...
void __not_in_flash_func(dma_handler_VGA)() {
    ...
    case VGA640x480_text_80_30:
        ...
        int i_ch=line_active/font_H; //номер символа в буфере в начале отобоажаемой строки
        int sh_ch=line_active%font_H;//"слой" символа


        uint8_t* line_tex_buf=&text_buf[i_ch*chrs_in_line]; //указатель откуда начать считывать символы
        uint8_t* line_tex_buf_color=&text_buf_color[i_ch*chrs_in_line];//указатель откуда начать считывать цвета символов
        ...
        for(int i=0;i<chrs_in_line;i++)
        {
            ...
            //считываем из быстрой палитры начало таблицы быстрого преобразования 2-битных комбинаций цветов пикселей
              uint16_t* fast_color=&txt_palette_fast[4*(*line_tex_buf_color++)];
...
void draw_text(char *string, int x, int y, uint8_t color, uint8_t bgcolor) 
{
    ...
    uint8_t* t_buf_c=text_buf_color+(text_buf_width*y)+x;
